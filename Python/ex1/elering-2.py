...
# Teeme For tsükliga kolme aasta, kuu, päeva andmed
for year in range(2023, 2026):
    for month in range(1, 13):
        start = f"{year}-{month:02}-01T00:00:01.999Z"
        end = f"{year}-{month:02d}-T28:59:59.999Z"
        data = get_price(start, end)
        save_json(data, f"elering_price_{year}_{month:02}.json")
